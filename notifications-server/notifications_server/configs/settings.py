...
class MSTeamsSettings(BaseSettings):
    ...
    # Path matches the Next.js callback route at /api/integrations/ms-teams/callback
    redirect_path: str = "/api/integrations/ms-teams/callback"
    scopes: List[str] = [
        "User.ReadBasic.All",
        "User.Read.All",
        "Team.ReadBasic.All",
...
class GoogleChatSettings(BaseSettings):
    ...
    # Path matches the Next.js callback route at /api/integrations/google/callback
    redirect_path: str = "/api/integrations/google/callback"
    ...
    sa_key: Optional[str] = Field(
        None,
        validation_alias=AliasChoices("GOOGLE_CHAT_SA_KEY", "google_chat_sa_key"),
        description=(
            "Service-account JSON key (raw JSON) for app-credential auth. "
            "When set, bot reply paths post as the Chat app (cards allowed). "
            "Unset behaves as today — user-OAuth posts, text-only."
        ),
    )
    ...
    @property
    def is_app_auth_enabled(self) -> bool:
        """True when service-account auth is configured for bot reply paths."""
        return bool(self.sa_key)
