...
//   - api-server/services/integrations/ssh.go
...
// sshHostRe and sshUserRe gate caller-supplied host_name / user_name overrides
// for RelayJobSSH commands. The values are interpolated into a shell command
// in the SSH case below, so anything they accept must be safe in that
// position. Patterns are duplicated from
// `relay-server/pkg/server/handlers/workspace.go` (the other site that
// constructs the same ssh command) — kept in sync by convention; both modules
...
func ExecuteContainerJob(toolContext core.NbToolContext, module RelayJob, query string, accountId string, configs map[string]any, raw bool) (any, error) {
	...
	switch module {
	case RelayJobSSH:
		// Resolution order per the integration field descriptions:
		//   (3) env-var defaults from the mounted k8s secret (SSH_USER / SSH_HOST)
		//   (2) integration's saved `host` / `username` config values
		//   (1) per-command host_name / user_name passed in `configs`
		// Any value that lands in the shell command is regex-checked —
		// including saved DB values (defense in depth).
		sshUser := "$SSH_USER"
		sshHost := "$SSH_HOST"
		if savedHost := strings.TrimSpace(getConfigValue(toolContext.ToolConfig.Values, "host")); savedHost != "" {
			if !sshHostRe.MatchString(savedHost) {
				return nil, fmt.Errorf("invalid saved host %q in integration config", savedHost)
			}
			sshHost = savedHost
		}
		if savedUser := strings.TrimSpace(getConfigValue(toolContext.ToolConfig.Values, "username")); savedUser != "" {
			if !sshUserRe.MatchString(savedUser) {
				return nil, fmt.Errorf("invalid saved username %q in integration config", savedUser)
			}
			sshUser = savedUser
		}
		if hn, ok := configs["host_name"].(string); ok && hn != "" {
			if !sshHostRe.MatchString(hn) {
				return nil, fmt.Errorf("invalid host_name %q: must be a hostname or IPv4 address", hn)
			}
			sshHost = hn
		}
		if un, ok := configs["user_name"].(string); ok && un != "" {
			if !sshUserRe.MatchString(un) {
				return nil, fmt.Errorf("invalid user_name %q: must start with a letter or underscore and contain only alphanumerics, dot, underscore, hyphen", un)
			}
			sshUser = un
		}
		// NOTE: keep the `mkdir / echo $SSH_KEY / ssh user@host '<cmd>'`
		// template in sync with the two other construction sites:
		//   - api-server/services/integrations/ssh.go (executeInternal)
		//   - collector-server/.../workspace.go (case "ssh" in buildWorkspaceAction)
		//
		// `sshShellQuote` single-quotes `query` so the workspace pod's local
		// shell can't re-parse it before ssh transmits it. Without this,
		// `$VAR` / `$1` / `$(...)` / backticks in the user's command get
		// expanded locally rather than on the remote host (e.g.
		// `awk '{print $1}'` was being delivered as `awk '{print }'`).
		//
		// `-o ConnectTimeout=10` surfaces unreachable-host failures within
		// 10s instead of waiting for the 60s pod-execution timeout.
		query = fmt.Sprintf(`mkdir -p ~/.ssh && echo "$SSH_KEY" > ~/.ssh/id_rsa && chmod 600 ~/.ssh/id_rsa && ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o LogLevel=ERROR -o ConnectTimeout=10 %s@%s %s`, sshUser, sshHost, sshShellQuote(query))
	case RelayJobPostgres:
		pgFlags := ""
		if dbName, ok := configs["database"]; ok {
			if dbNameStr, ok := dbName.(string); ok && dbNameStr != "" {
