...
query listAutoOptimize {
  autooptimize_list(where:__WHERE__) {
    rows {
      id
      category
      status
...
mutation ApplyRecommendation($account_id: String!, $recommendation_id: String!, $data: jsonb, $provider: String, $provider_config: ProviderConfig) {
  recommendations_apply(object: {account_id: $account_id, recommendation_id: $recommendation_id, data: $data, provider: $provider, provider_config: $provider_config }) {
    data
  }
}
`;
...
mutation ApplyRecommendation($account_id: String!, $event_id: String!, $data: jsonb, $provider: String, $provider_config: ProviderConfig) {
  events_update_resolution(object: {account_id: $account_id, event_id: $event_id, data: $data, provider: $provider, provider_config: $provider_config }) {
    data
  }
}
`;
...
mutation NodeRecommendation($account: String!, $graviton: Boolean!, $instance_groups: [String!]!, $tenant_id: String!, $number_of_recommendations: Int) {
  ml_generate_node_recommendations(account: $account, graviton: $graviton, preferred_instance_groups: $instance_groups, tenant: $tenant_id, number_of_recommendations: $number_of_recommendations) {
    data
  }
}
`;
...
const apiRecommendations = {
  async getAutoOptimize(query: any) {
    ...
    try {
      ...
      const autoPilotRows = response?.data?.data?.autooptimize_list?.rows?.map((item: any) => ({
        ...item,
        auto_optimize_resource_maps:
  ...
  async listRecommendationNamesapces({
    ...
    `;
      ...
      query list_namespaces($accountId: String!${withStatus ? ', $status: String!' : ''}${withRuleName ? ', $ruleName: String!' : ''}) {
        recommendation: recommendations_list(where: {
