...
query ListAcc($limit: Int, $offset: Int) {
  accounts_list(where: __WHERE__, order_by: [{column: "created_at", order: desc}] limit: $limit, offset: $offset) {
    rows {
      account_name
      account_number
      id
  ...
  accounts_aggregate(where: __WHERE__){
    rows{
      count
    }
  }
...
query ListAWSAcc {
  cloud_accounts: accounts_list(where: {cloud_provider: {_eq: "AWS"}}) {
    rows {
      account_name
      id
      created_at
...
query ListGCPAcc {
  cloud_accounts: accounts_list(where: {cloud_provider: {_eq: "GCP"}}) {
    rows {
      account_name
      id
      created_at
...
query AccountAgentHealth {
  agent: agents_list_health(where: __WHERE__) {
    rows {
      cloud_account_id
      last_connected_at
      k8s_version
...
mutation AwsEventBridgeOnboard($object: AwsEventBridgeOnboardInput!) {
  aws_get_onboard_eventbridge_url(object: $object) {
    url
    external_id
  }
}
...
mutation AzureEventGridOnboard($object: AzureEventGridOnboardInput!) {
  azure_get_onboard_eventgrid_url(object: $object) {
    url
    external_id
    webhook_url
  }
...
mutation TriggerAnomalyExecute($request: TriggerAnomalyExecuteRequest!) {
  anomaly_execute(request: $request) {
    status
    message
  }
}
...
query FlightCheck($where: RecommendationWhereRequest) {
  recommendation: recommendations_list(where: $where) {
    rows {
      recommendation
      created_at
      updated_at
...
const apiKubernetes1 = {
    ...
    try {
      ...
      for (const id of accountIds) {
        if (id == 'demo') {
          ...
          return { demo: response['GetInsights'].data.insights_aggregate.data };
        }
        const LIST_K8s_INSIGHTS = `
          query ListK8sInsights {
            insights_list(where:__WHERE__) {
              rows {
                applications
                source
                tenant_id
        ...
        insights[id] = (response?.data?.data?.insights_list?.rows ?? []).map((item: any) => ({
          ...item,
  ...
  async listK8sAnomaliesData(data: any) {
    ...
    query ListK8sAnomaliesData(
      ...
      anomalies_list_v2(where: __WHERE__, offset: $offset, limit: $limit, order_by: {column: "evaluated_at", order: desc}) {
        rows {
          anomaly_type
          updated_at
          config_id
  ...
  async listK8sAnomalies(data: any) {
    ...
    query ListK8sAnomalies(
      ...
      anomalies_list(where: __WHERE__, offset: $offset, limit: $limit) {
        rows {
          anomaly_type
          name
          namespace
  ...
  async listK8sAnomaliesCount(data: any) {
    ...
    query K8sAnomaliesCount {
      anomalies_list(where: __WHERE__) {
        rows {
          count
        }
      }
  ...
  async getAgentPlaybookOfEvent(data: any) {
    ...
    query GetAgentPlaybookOfEvent {
      agents_list_playbooks(where: __WHERE__) {
        rows {
          id
          trigger_params
          alert_name
    ...
    try {
      ...
      if (response?.data?.data?.agents_list_playbooks?.rows) {
        response.data.data.agent_playbook = response.data.data.agents_list_playbooks.rows;
      }
      return response;
    } catch (error) {
  ...
  async applicationProfileConvert(data: any) {
    ...
    mutation ApplicationProfileConvert {
      applications_convert_profile(request: __WHERE__) {
        data {
          svg_profile
        }
      }
  ...
  async applicationProfile(data: any) {
    ...
    mutation ApplicationProfile {
      applications_execute_profile(request: __WHERE__) {
        data {
          status
          base64_profile
          error_message
  ...
  async applicationProfileStatus(data: any) {
    ...
    mutation GetApplicationProfile {
      applications_get_profile_status(request: __WHERE__) {
        data {
          status
          base64_profile
          error_message
  ...
  async applicationProfileHistory(data: any) {
    ...
    query ApplicationProfileHistory {
      applications_list_profiles(where: __WHERE__, order_by: {order: desc, column: "updated_at"}) {
        rows {
          pod_name
          workload_name
          namespace
  ...
  setUpgradePlanTaskStatus: async function (accountId: string, planId: string, stepId: string, taskId: string, status: string) {
    ...
    try {
      const query = `mutation SetUpgradePlanTaskStatus($accountId: String!, $planId: String!, $stepId: String!, $taskId: String!, $status: String) {
        upgradeplans_upsert_task(account_id: $accountId, plan_id: $planId, step_id: $stepId, task_id: $taskId, status: $status) {
          id
          status
        }
      }
  ...
  setUpgradePlanTaskOwner: async function (accountId: string, planId: string, stepId: string, taskId: string, owner: string) {
    ...
    try {
      const query = `mutation SetUpgradePlanTaskOwner($accountId: String!, $planId: String!, $stepId: String!, $taskId: String!, $owner: String) {
        upgradeplans_upsert_task(account_id: $accountId, plan_id: $planId, step_id: $stepId, task_id: $taskId, owner: $owner) {
          id
          owner
        }
      }
  ...
  getClusterHealth: async function (accountId: string, resourceType: string) {
    ...
    try {
      const query = `query CheckClusterHealth($accountId: String!, $resourceType: String!) {
        clusters_check_health(account_id: $accountId, resource_type: $resourceType) {
          account_id
          nodes
          persistentVolumes
          services
      ...
      return {
        res: response?.data?.data?.clusters_check_health,
  ...
  executeClusterUpgradePlannerCommand: async function (
    ...
    if (accountId === 'demo')
      return {
        data: {
          data: null,
          errors: [{ message: 'Command execution is not supported for Demo account.' }],
        },
      };
...
`;
  ...
  utilisationApi: async function (query: any) {
    ...
    query MetricsQueryUtilisation($jsonFilter: jsonb!, $accountId: String!, $startTime: Float!, $endTime: Float!) {
      metrics_aggregate_utilisation(request: {account_id: $accountId, request: $jsonFilter, end_time: $endTime, start_time: $startTime}) {
        results
      }
    }    
    `;
    if (query.accountId === 'demo' || query.account_id === 'demo') return [];
    const data: any = {};
    ...
    try {
      ...
      const metricsResults = response?.data?.data?.metrics_aggregate_utilisation?.results || [];
