...
query listK8ClusterData {
  ...
  cloud_accounts: accounts_list(where:{cloud_provider:{_eq:"K8s"}, status:{_eq:"active"}}){
    rows {
      id
      account_name
    }
...
query list_k8_issues_name($limit:Int, $offset:Int) {
  events: events_list(where: __WHERE__, order_by: [{column:"starts_at",  order:desc}], limit: $limit, offset: $offset) {
    rows{
      title
      subject_name
      id
...
query list_k8_pod_exceptions($limit:Int,$aggregationKey: [String!], $startDate: Datetime, $endDate: Datetime) {
  events: events_list(order_by: [{column: "starts_at", order: desc}],
...
mutation singleConfigAuotPilot($data: autooptimize_create!) {
  autopilot_insert_one: autooptimize_create(arg1: $data) {
    id
  }
}
`;
...
mutation TriggerCloudAccountSync($account_id: String!) {
  accounts_sync(account_id: $account_id) {
    success
    message
  }
}`;
...
const apiKubernetes = {
 ...
 query list_k8_issues_data($limit:Int, $offset:Int) {
  ...
  events: events_list(where: __WHERE__, order_by: __ORDER_BY__, limit: $limit, offset: $offset) {
    rows{
      account_id
      subject_type
      subject_name
...
`;
  ...
  async getInsights(accountId: string[], source: string[], resource_ids: string[]) {
    try {
      ...
      return {
        data: parseInsightJsonFields(response?.data?.data?.insights_list?.rows),
 ...
 `;
  ...
  async tracesServiceMap(data: any) {
    ...
    mutation ServiceMapViaTraces {
      traces_get_service_map(request: __WHERE__) {
        data {
          labels
          applications {
            Id {
  ...
  async triggerCloudSync(accountId: string) {
    try {
      ...
      return { data: response?.data?.data?.accounts_sync, errors: response?.data?.errors };
    } catch (error) {
