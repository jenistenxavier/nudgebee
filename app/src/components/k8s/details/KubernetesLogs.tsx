...
const KubernetesLogs: React.FC<KubernetesLogProps> = ({
  ...
  const handleLogQueryFromDrilldown = useCallback(
    (item: any) => {
      // Datadog has no Builder mode, so chip-state isn't rendered into the
      // query string. Inline the filter directly into logQuery using the
      // Datadog tag syntax (`key:"value"` / `-key:"value"`). K8s log labels
      // surface as Datadog tags, not log attributes (`@`-prefix is reserved
      ...
      if (logProvider === 'datadog') {
        ...
        const toAdd = include ? positive : negative;
        const toRemove = include ? negative : positive;
        setLogQuery((prev) => {
          const tokens = prev.trim() ? prev.trim().split(/\s+/) : [];
          const filtered = tokens.filter((tok) => tok !== toAdd && tok !== toRemove);
          filtered.push(toAdd);
    ...
    (allResults: any[]) => {
      const result: any[] = allResults.map((res: any) => {
        ...
        return [
          ...
          {
            component: (
              <Box display={'flex'} justifyContent={'flex-end'} gap='var(--ds-space-1)'>
                <DsButton
                  tone='secondary'
                  size='xs'
                  composition='icon-only'
                  aria-label={`Ask ${assistantName}`}
                  icon={<SafeIcon alt={`Ask ${assistantName}`} src={getNubiIconUrl()} width={20} height={20} />}
                  onClick={(event) => {
                    event.stopPropagation();
                    handleGenerateLogAnalysis(res, res.message);
                  }}
                />
                <ThreeDotsMenu menuItems={menuItems} data={{ stream: res, data: res.message }} onMenuClick={onMenuClick} />
              </Box>
            ),
          },
        ];
    ...
    <div>
      ...
      <ListingLayout id='query-logs'>
              ...
              {showPolling && (
                <>
                  <FilterDropdown
                    ...
                    label='Limit'
                    value={logLimit}
                    onSelect={(e) => setLogLimit(Number(e?.target?.value))}
                    options={[
                      { value: 50, label: '50' },
                      { value: 100, label: '100' },
                      { value: 200, label: '200' },
        ...
        >
          ...
          {showQueryTextBox && qLEditor === 'code' && logProvider === 'ES' && (
            <Tooltip
              variant='explainer'
              title='Elasticsearch Query DSL'
              desc='Uses Elasticsearch Query DSL to write and execute log queries.'
              placement='top'
              tooltipStyle={{ maxWidth: ds.space.mul(0, 160) }}
            >
