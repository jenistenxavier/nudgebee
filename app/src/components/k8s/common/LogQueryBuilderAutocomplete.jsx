...
// Number of filter chips shown inline before the rest collapse into a +N tooltip.
const CHIP_LIMIT = 6;
// Max chars rendered inside a chip before its label truncates and reveals on hover.
const CHIP_CHAR_LIMIT = 40;
...
const TruncatedTooltip = ({ title, children }) => {
  ...
  return (
    <DsTooltip
      ...
      componentsProps={{ popper: { style: { zIndex: 1600 } } }}
    >
      <span
...
const LogQueryBuilderAutocomplete = ({
      ...
      const fetchMetrics = async () => {
        ...
        try {
          ...
          if (cachedPrometheusLabels) {
            ...
            const metricsList = res?.data?.data?.metrics_list_names?.map((m) => m.metric) || [];
      ...
      const fetchIndexes = async () => {
        ...
        try {
          ...
          if (cachedESIndexes) {
            ...
            const indexList = res?.data?.data?.metrics_list_names?.map((m) => m.metric) || [];
            if (indexList.length) {
  ...
  const getStepIcon = () => {
    ...
    switch (currentStep) {
      case 'label':
        return <LabelIcon sx={{ fontSize: 'var(--ds-text-heading)' }} color='success' />;
      case 'operator':
        return <FilterIcon sx={{ fontSize: 'var(--ds-text-heading)' }} color='warning' />;
      case 'value':
        return <ValueIcon sx={{ fontSize: 'var(--ds-text-heading)' }} color='success' />;
      default:
        return <SearchIcon sx={{ fontSize: 'var(--ds-text-heading)' }} color='success' />;
    }
  };
  ...
  return (
    <Box sx={{ width: '100%', minHeight: height, display: 'flex', flexDirection: 'column', gap: ds.space[4] }}>
      {/* Query Blocks */}
      {queryBlocks.map((block, index) => (
        <Box key={block.id} sx={{ display: 'flex', flexDirection: 'row', gap: ds.space[4], width: '100%' }}>
          ...
          >
            <Box display={'flex'} flexDirection={'row'} gap={ds.space.mul(0, 3)} width={'90%'} alignItems={'center'}>
              {allowMultipleQueries && (
                <Typography variant='caption' sx={{ fontWeight: 'var(--ds-font-weight-semibold)', color: 'var(--ds-gray-700)' }}>
            ...
            <Box display='flex' flexDirection='column' gap='var(--ds-space-2)' width='100%'>
              {/* Top Row */}
              <Box display='flex' flexDirection='row' gap={ds.space.mul(0, 3)} width='100%'>
                {(((logProvider === 'prometheus' ||
                  logProvider === 'datadog' ||
                  logProvider === 'newrelic' ||
                  logProvider === 'dynatrace' ||
                            ...
                            <ListItem key={`${suggestion.type}-${suggestion.value}`} disablePadding>
                              <ListItemButton
                                ...
                                sx={{
                                  ...
                                  gap: 'var(--ds-space-2)',
                                  minWidth: 0,
                                  backgroundColor: index === selectedIndex ? 'var(--ds-overlay-item-hover-bg)' : 'transparent',
                              ...
                              >
                                <Box component='span' sx={{ display: 'inline-flex', alignItems: 'center', color: 'inherit', flexShrink: 0 }}>
                                  {suggestion.type === 'label' && <LabelIcon sx={{ fontSize: 'var(--ds-text-title)' }} color='success' />}
                                  {suggestion.type === 'operator' && <FilterIcon sx={{ fontSize: 'var(--ds-text-title)' }} color='warning' />}
                                  {suggestion.type === 'value' && <ValueIcon sx={{ fontSize: 'var(--ds-text-title)' }} color='success' />}
                                </Box>
                                <ListItemText
                                  primary={
                                    <TruncatedTooltip title={suggestion.label ?? suggestion.value}>
                                  ...
                                  secondaryTypographyProps={{ fontSize: 'var(--ds-text-caption)', color: 'var(--ds-gray-500)', noWrap: true }}
                                  sx={{ overflow: 'hidden' }}
                                />
                              </ListItemButton>
                            </ListItem>
                  ...
                  {activeBlockId === block.id && (
                    <DsTooltip
                      ...
                      title={
                        <Box>
                          <Typography
                            sx={{ fontSize: 'var(--ds-text-body)', fontWeight: 'var(--ds-font-weight-semibold)', color: 'var(--ds-gray-700)', mb: 0 }}
                          >
                            Filter by {heading}
                          </Typography>
                          ...
                          <Box
                            sx={{
                              mt: ds.space[3],
                              pt: ds.space[3],
                              borderTop: '1px solid var(--ds-blue-300)',
                            }}
                          >
                            <Typography
                              sx={{
                    ...
                    >
                      <Box component='span' sx={{ display: 'inline-flex', opacity: 0.4 }}>
                        <DsButton tone='ghost' size='xs' composition='icon-only' aria-label='Filter help' icon={<InfoIcon />} />
                      </Box>
                    </DsTooltip>
                  )}
                </Box>
          ...
          {['loki', 'newrelic', 'dynatrace', 'loggly'].includes(logProvider) && providerType !== 'metrics' && activeBlockId === block.id && (
            ...
            >
              <Typography variant='subtitle2' sx={{ mb: ds.space[2], fontWeight: 'medium', color: 'var(--ds-gray-700)' }}>
                Operations
              </Typography>
              {block.operations.map((op) => (
                <Box key={op.id} sx={{ display: 'flex', alignItems: 'center', gap: ds.space[2], mb: ds.space[2] }}>
                  <FilterDropdown
                  ...
                  <Box sx={{ flex: 1 }}>
                    <Input size='sm' placeholder='Enter value' value={op.value} onChange={(next) => updateOperation(op.id, 'value', next)} />
                  </Box>
                  ...
                  <DsButton
                    ...
                    icon={<DeleteIconMui fontSize='small' />}
              ...
              <Box>
                ...
                {block.queryItems.length === 0 && !(logProvider === 'dynatrace' && providerType === 'logs') && (
                  <Box sx={{ display: 'flex', alignItems: 'center', mt: ds.space[2], gap: ds.space[1] }}>
      ...
      {allowMultipleQueries && logProvider !== 'ES' && (
        <Box
          sx={{ display: 'flex', justifyContent: 'flex-start', mt: ds.space[4], flexDirection: 'column', gap: ds.space[2], alignItems: 'flex-start' }}
        >
          <DsButton
            tone='primary'
            size='md'
