...
import NodeDetails from '@components/k8s/details/NodeDetails';
...
const EDGE_STYLE = { stroke: 'var(--ds-gray-400)', strokeWidth: 1.5, cursor: 'pointer' };
const EDGE_MARKER_END = { type: MarkerType.ArrowClosed, color: 'var(--ds-gray-400)' };
const HANDLE_STYLE_VISIBLE = { background: 'var(--ds-gray-600)' };
    ...
    return (
      <div className={`service-node-wrapper ${isZoomedOut ? 'lod-dot' : 'lod-full'}`}>
        ...
        <div className='service-node' style={{ borderColor }}>
          ...
          <Button
            className='focus-btn'
            icon={<FilterCenterFocusIcon style={{ fontSize: ds.text.bodyLg }} />}
            tone='secondary'
            size='xs'
            tooltip='Focus on this node'
...
const NodeBreadcrumbs = memo(({ fullPath, onNavigateToNode, onRemoveNode }) => {
  ...
  return (
    <Box sx={{ borderBottom: '1px solid var(--ds-gray-200)', backgroundColor: 'var(--ds-background-200)' }}>
      {/* Main breadcrumb bar */}
      <Box
        sx={{
          display: 'flex',
          ...
          flexWrap: 'wrap',
          minHeight: ds.space.mul(0, 18),
        }}
      >
        <Typography sx={{ fontSize: 'var(--ds-text-small)', color: 'var(--ds-gray-600)', marginRight: 'var(--ds-space-1)' }}>Path:</Typography>
        {fullPath.map((node, index) => (
          <Box key={`${node.value}-${index}`} sx={{ display: 'flex', alignItems: 'center', gap: 'var(--ds-space-1)' }}>
            {index > 0 && <ChevronRightIcon sx={{ fontSize: ds.text.title, color: 'var(--ds-gray-400)' }} />}
            <Tooltip
            ...
            >
              <Box
                ...
                sx={{
                  ...
                  backgroundColor: node.isUserSelected
                    ? index === fullPath.length - 1
                      ? 'var(--ds-blue-100)'
                      : 'var(--ds-background-300)'
                    : 'transparent',
                  border: node.isUserSelected ? 'none' : '1px dashed var(--ds-gray-300)',
                  cursor: 'pointer',
                  opacity: node.isUserSelected ? 1 : 0.7,
                  '&:hover': { backgroundColor: 'var(--ds-gray-200)', opacity: 1 },
                }}
              >
                <Typography
                  sx={{
                ...
                {node.isUserSelected && (
                  <CloseIcon
                    ...
                    sx={{ fontSize: ds.text.bodyLg, color: 'var(--ds-gray-600)', cursor: 'pointer', '&:hover': { color: 'var(--ds-gray-700)' } }}
                  />
                )}
              </Box>
            </Tooltip>
        ...
        {hasEdges && fullPath.length > 1 && (
          <Box
            ...
            sx={{
              ...
              '&:hover': { backgroundColor: 'var(--ds-gray-200)' },
            }}
          >
            <InfoOutlinedIcon sx={{ fontSize: ds.text.title, marginRight: 'var(--ds-space-1)' }} />
            <Typography sx={{ fontSize: 'var(--ds-text-caption)' }}>Details</Typography>
            {isExpanded ? <ExpandLessIcon sx={{ fontSize: ds.text.title }} /> : <ExpandMoreIcon sx={{ fontSize: ds.text.title }} />}
          </Box>
        )}
      </Box>
      ...
      {isExpanded && hasEdges && (
        ...
        >
          <Typography sx={{ fontSize: 'var(--ds-text-caption)', color: 'var(--ds-gray-600)', marginBottom: 'var(--ds-space-1)' }}>
            Full path with relationships:
          </Typography>
          <Box sx={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 'var(--ds-space-1)' }}>
            {fullPath.map((node, index) => (
              <Box key={`detail-${node.value}-${index}`} sx={{ display: 'flex', alignItems: 'center', gap: 'var(--ds-space-1)' }}>
                {index > 0 && node.edgeFromPrev && (
                  ...
                  >
                    <Typography sx={{ fontSize: 'var(--ds-text-caption)', color: 'var(--ds-gray-600)' }}>—[</Typography>
                    <Typography sx={{ fontSize: 'var(--ds-text-caption)', color: 'var(--ds-blue-600)', fontWeight: 'var(--ds-font-weight-medium)' }}>
                      {node.edgeFromPrev}
                    </Typography>
                    <Typography sx={{ fontSize: 'var(--ds-text-caption)', color: 'var(--ds-gray-600)' }}>]→</Typography>
                  </Box>
                )}
                {index > 0 && !node.edgeFromPrev && <ChevronRightIcon sx={{ fontSize: ds.text.bodyLg, color: 'var(--ds-gray-400)' }} />}
                <Tooltip title={<BreadcrumbTooltipContent node={node} />} placement='bottom'>
                  <Box
                    sx={{
                      padding: 'var(--ds-space-1) var(--ds-space-2)',
                      borderRadius: 'var(--ds-radius-sm)',
                      backgroundColor: node.isUserSelected ? 'var(--ds-blue-100)' : 'var(--ds-background-100)',
                      border: node.isUserSelected ? '1px solid var(--ds-blue-300)' : '1px dashed var(--ds-gray-300)',
                      cursor: 'pointer',
                      '&:hover': { backgroundColor: 'var(--ds-gray-200)' },
                    }}
                  >
    ...
    >
      {node.accountName && (
        <Box sx={{ display: 'flex', gap: 'var(--ds-space-2)' }}>
          <Typography sx={{ fontSize: 'var(--ds-text-caption)', color: 'var(--ds-gray-400)', minWidth: ds.space.mul(0, 35) }}>Account:</Typography>
      ...
      <Box sx={{ display: 'flex', gap: 'var(--ds-space-2)' }}>
        <Typography sx={{ fontSize: 'var(--ds-text-caption)', color: 'var(--ds-gray-400)', minWidth: ds.space.mul(0, 35) }}>Name:</Typography>
        <Typography sx={{ fontSize: 'var(--ds-text-caption)', color: 'var(--ds-background-100)', wordBreak: 'break-word' }}>
          {node.name || '-'}
        </Typography>
      </Box>
      <Box sx={{ display: 'flex', gap: 'var(--ds-space-2)' }}>
        <Typography sx={{ fontSize: 'var(--ds-text-caption)', color: 'var(--ds-gray-400)', minWidth: ds.space.mul(0, 35) }}>Type:</Typography>
        <Typography sx={{ fontSize: 'var(--ds-text-caption)', color: 'var(--ds-background-100)' }}>
          {snakeToTitleCase(node.nodeType || '')}
        </Typography>
      </Box>
      <Box sx={{ display: 'flex', gap: 'var(--ds-space-2)', alignItems: 'flex-start' }}>
        <Typography sx={{ fontSize: 'var(--ds-text-caption)', color: 'var(--ds-gray-400)', minWidth: ds.space.mul(0, 35) }}>Unique Key:</Typography>
        <Typography
          sx={{
...
const ServiceMapContent = () => {
  ...
  // Tenant-level emptiness: filter-options API returned zero nodes AND the user
  ...
  // Node selection is intentionally NOT included: the value dropdown mirrors the
  // key dropdown (which ignores the Node picker), and a node selection scopes
  // values to seeds only — inconsistent with the seeds+neighbors the graph shows.
  ...
  return (
    <>
      ...
      <Modal
        ...
        title='Edge Details'
        contentStyles={{ padding: 'var(--ds-space-4)' }}
      >
        {isEdgeDetailsLoading ? (
          <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: ds.space.mul(0, 75) }}>
            <CircularProgress size={40} />
          </Box>
        ) : (
      ...
      <Box
        ...
        gap='var(--ds-space-2)'
        sx={{ height: 'calc(100vh - 148px)', overflow: 'hidden', padding: 'var(--ds-space-2) 0px', isolation: 'isolate' }}
      >
        ...
        <Box
          sx={{
            ...
            boxShadow: '0px 4px 4px 0px var(--ds-gray-alpha-200)',
        ...
        >
          {(filterHistory.length > 0 || filterFuture.length > 0 || selectedNodes.length > 0) && (
            <Box sx={{ display: 'flex', alignItems: 'center', gap: ds.space[2], mb: ds.space[2] }}>
              {filterHistory.length > 0 && (
                <Button data-testid='kg-back-btn' tone='secondary' size='sm' icon={<ArrowBackIcon sx={{ width: '14px' }} />} onClick={handleBack}>
                  Back
                </Button>
              )}
              {filterFuture.length > 0 && (
                <Button
                  ...
                  tone='secondary'
                  size='sm'
                  icon={<ArrowForwardIcon sx={{ width: '14px' }} />}
                  iconPlacement='end'
          ...
          <Box
            sx={{ position: 'absolute', top: ds.space[3], right: ds.space[3], zIndex: 200, display: 'flex', alignItems: 'center', gap: ds.space[2] }}
          >
            ...
            <FilterDropdown
              ...
              popoverWidth={ds.space.mul(0, 210)}
              popoverAlign='center'
              size='sm'
              onSelect={(event) => {
            ...
            <Tooltip
              ...
              tooltipStyle={{ maxWidth: ds.space.mul(0, 200), padding: 'var(--ds-space-4)' }}
            >
              <span>
                <Button id='relationship-types-btn' tone='secondary' size='sm' icon={<AccountTreeOutlinedIcon sx={{ width: '16px' }} />}>
                  Relationships
                </Button>
              </span>
            </Tooltip>
            ...
            {isTenantAdmin() && (
              <Button
                ...
                tone='secondary'
                size='sm'
                icon={<SettingsOutlinedIcon sx={{ width: '16px' }} />}
          ...
          <Box ref={graphWrapperRef} sx={{ flex: 1, position: 'relative' }}>
            ...
            {focusedNodeId && (
              <Box
                ...
                sx={{
                  ...
                  boxShadow: `0 ${ds.space[0]} ${ds.space[2]} rgba(0,0,0,0.2)`,
                  cursor: 'default',
                  userSelect: 'none',
                }}
              >
                <FilterCenterFocusIcon sx={{ fontSize: ds.text.bodyLg }} />
                <Typography
                  variant='caption'
                  sx={{ color: 'var(--ds-background-100)', fontWeight: 'var(--ds-font-weight-medium)', fontSize: 'var(--ds-text-small)' }}
                >
                  Focus: {nodeDataMapRef.current.get(focusedNodeId)?.name || focusedNodeId}
                </Typography>
                <CloseIcon
                  data-testid='kg-focus-exit'
                  sx={{ fontSize: ds.text.bodyLg, cursor: 'pointer', ml: ds.space[0], opacity: 0.85, '&:hover': { opacity: 1 } }}
            ...
            {isGraphLoading ? (
                ...
                >
                  ...
                  <Box
                    sx={{
                      ...
                      borderRadius: 'var(--ds-radius-pill)',
                      background: 'linear-gradient(135deg, var(--ds-amber-100) 0%, var(--ds-amber-200) 100%)',
                      display: 'flex',
                      alignItems: 'center',
                      justifyContent: 'center',
                    }}
                  >
                    <WarningAmberRoundedIcon sx={{ fontSize: 32, color: 'var(--ds-amber-500)' }} />
                  </Box>
                  ...
                  <Box sx={{ display: 'flex', flexDirection: 'column', gap: 'var(--ds-space-2)' }}>
                    ...
                    >
                      ...
                      <Typography
                        component='span'
                        sx={{ fontWeight: 'var(--ds-font-weight-semibold)', color: ds.gray[700], fontSize: 'var(--ds-text-body)' }}
                      >
              ...
              <Box
                data-testid='kg-empty-populating'
                display='flex'
                flexDirection='column'
                alignItems='center'
                justifyContent='center'
                ...
                >
                  <Box
                    sx={{
                      ...
                      alignItems: 'center',
                      justifyContent: 'center',
                      '@keyframes kgEmptyPulse': {
                        '0%, 100%': { transform: 'scale(1)', opacity: 1 },
                        '50%': { transform: 'scale(1.06)', opacity: 0.85 },
                      },
                      animation: 'kgEmptyPulse 2.4s ease-in-out infinite',
                    }}
                  >
                    <AutoAwesomeRoundedIcon sx={{ fontSize: 32, color: 'var(--ds-blue-500)' }} />
                  </Box>
                  ...
                  <Box sx={{ display: 'flex', flexDirection: 'column', gap: 'var(--ds-space-2)' }}>
                    ...
                    >
                      We&apos;re discovering services, dependencies and infrastructure across your environment. This usually takes a few minutes after
                    ...
                    >
                      Check back shortly — the graph will appear here automatically once data starts flowing in.
                    </Typography>
                  </Box>
                </Box>
              </Box>
