...
import { sanitizeTaskId, validateTaskId } from '@components/workflow/utils/taskUtils';
import { cleanupSwitchReferencesAfterDelete, renameTaskReferencesInNodes } from '@components/workflow/utils/templateUtils';
import { spliceEdgesOnNodeDelete } from '@components/workflow/utils/spliceNode';
...
const ActionNode = ({ id, data, isConnectable, selected, onTestTask, accountId, onAddFromHandle }: any) => {
      ...
      {!getExecutionIcon() && getValidationIcon() && (
        <div
          style={{
            ...
            padding: 'var(--ds-space-1)',
            color: data.serverError ? '#dc2626' : data.taskConfig?.valid === false ? '#dc2626' : '#16a34a',
          }}
        >
          {getValidationIcon()}
        </div>
  ...
  const getBorder = (): string => {
    ...
    if (data.serverError) return '2px solid #dc2626';
