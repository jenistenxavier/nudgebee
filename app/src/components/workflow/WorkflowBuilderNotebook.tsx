...
const NubiChatSidebar = lazy(() => import('@shared/layout/NubiChatSidebar'));
...
const WorkflowBuilderNoteBook: React.FC<WorkflowBuilderNotebookProps> = ({ mode }) => {
  ...
  // Which kind of run was requested:
  //   'current' — snapshot the on-screen draft (use_draft_definition=true).
  //              User stays on the editor.
  //   'live'    — trigger the published live version. After success, navigate
  ...
  const [pendingExecutionId, setPendingExecutionId] = useState<string | undefined>(undefined);
  ...
  useEffect(() => {
    ...
    if (hash && (hash === 'editor' || hash === 'json' || hash === 'executions')) {
      // Read-only users have no editor access — coerce any editor/json hash to executions.
      if (!canEdit && hash !== 'executions') {
        setCurrentMode('executions');
        return;
      }
      setCurrentMode(hash);
    } else if (!canEdit) {
  ...
  // Guards the plain empty-workflow init from re-firing when `taskDefinitions` resolves
  ...
  useEffect(() => {
    ...
    if (isNewWorkflow) {
      ...
      // or if the plain empty workflow was already initialized this mount (prevents re-firing after
      // taskDefinitions loads and wiping a trigger node the user already placed — #30510).
      if (
        aiWorkflowInitializedRef.current ||
        templateInitializedRef.current ||
        eventTriggerInitializedRef.current ||
        newEmptyWorkflowInitializedRef.current
      ) {
        setLoading(false);
        return;
      }
  ...
  useEffect(() => {
    if (
      ...
      newEmptyWorkflowInitializedRef.current = false;
    }
  }, [
    isNewWorkflow,
    router.query.loadFromAI,
  ...
  const handleConfirmRestore = useCallback(async () => {
    ...
    try {
      ...
      snackbar.success(`Checked out version ${targetVersion}.`);
  ...
  const handleConfirmPublish = useCallback(async () => {
    ...
    try {
      ...
      const v = response?.data?.workflows_create_version as WorkflowVersionEntry | undefined;
  ...
  // `variant` is retained for the call signature; validation no longer differs
  // by variant now that the DRAFT status (and its run gate) has been removed.
  const validateBeforeExecution = useCallback(
    (mode: 'run' | 'dryrun', _variant: 'current' | 'live' = 'current'): string | null => {
  ...
  // `variant` is 'current' for the canvas-default "Run current" path (draft
  // snapshot, user stays on the editor) or 'live' for the secondary "Run
  // live version" menu item (published live version, navigates to the
  // executions tab on success).
  const handleRunButtonClick = useCallback(
    async (variant: 'current' | 'live' = 'current') => {
      setRunVariant(variant);
      try {
        const validationError = validateBeforeExecution('run', variant);
        if (validationError) {
  ...
  const prepareDryRunRequest = useCallback(
    (taskIdsToInclude?: string[], inputsOverride?: any) => {
      ...
      return {
        ...
        // so dry-run notifications show the automation name, not a placeholder
  ...
  const executeRun = useCallback(
    async (inputs: any) => {
      ...
      const response: any = await apiWorkflow.triggerWorkflow({
        ...
        inputs,
        use_draft_definition: variant === 'current',
      });
      ...
      if (triggerData?.execution_id) {
        ...
        fetchExecutionData();
        // On "Run live", jump to the Executions tab and auto-select the
        // new run so the user can immediately see the live-version canvas
        // and task progress. "Run current" keeps the user on the editor
        // so they can keep iterating.
        if (variant === 'live') {
          setPendingExecutionId(triggerData.execution_id);
          setCurrentMode('executions');
        }
    ...
    [workflowId, accountId, fetchExecutionData, runVariant]
  );
  ...
  const executeDryRun = useCallback(
    async (inputs: any) => {
      ...
      const result = response?.data?.workflow_dryrun_execute;
  ...
  // Defaults to "Run current" so the inline play button matches the canvas
  // primary action.
  const handleTestRun = useCallback(async () => {
    await handleRunButtonClick('current');
  ...
  // Same pattern for the trigger configuration sidebar — buffered pending config
  // surfaced when the user tries to close the trigger sidebar with unsaved edits.
  const [pendingTriggerDiscard, setPendingTriggerDiscard] = useState<{ nodeId: string; pendingConfig: { type: string; params: any } } | null>(null);

  // Close ActionDetailsSidebar when its selected node disappears (e.g. node
  // deleted from the canvas). The sidebar buffers localData internally, so
  // without this effect it would keep rendering the deleted node's stale
  // config — and the dirty-comparison could trip the unsaved-changes modal.
  useEffect(() => {
    if (actionDetailsSidebarOpen && !sidebarSelectedNode) {
      setActionDetailsSidebarOpen(false);
      setPendingDiscard(null);
    }
  }, [actionDetailsSidebarOpen, sidebarSelectedNode, setActionDetailsSidebarOpen]);
  ...
  return (
    ...
    >
      ...
      <WorkflowHeader
        ...
        activeTab={canEdit ? (currentMode === 'json' ? 'editor' : currentMode) : 'executions'}
        allowTitleEdit={canEdit}
        canEdit={canEdit}
        ...
        >
          ...
          {currentMode === 'executions' ? (
            ...
            <Box sx={{ height: '100%', width: '100%' }}>
              <Suspense fallback={<Loader style={{ height: '100%', width: '100%' }} />}>
                <ExecutionsView
                  ...
                  onPendingExecutionConsumed={() => setPendingExecutionId(undefined)}
                />
              </Suspense>
            </Box>
          ) : (
            ...
            <>
                      ...
                      {isNewWorkflow &&
                        ...
                        !eventTriggerInitializedRef.current && (
                          ...
                          >
                            ...
                            >
                              <Box
                                sx={{
                                  ...
                                  height: '36px',
                                  borderRadius: 'var(--ds-radius-xl)',
                                  background: 'linear-gradient(135deg, var(--ds-green-500) 0%, var(--ds-green-500) 100%)',
                                  display: 'flex',
                                  alignItems: 'center',
                                  justifyContent: 'center',
                            ...
                            <Box sx={{ display: 'flex', gap: 'var(--ds-space-3)', justifyContent: 'center' }}>
                              {(
                                [
                                  {
                                    key: 'manual',
                                ...
                                <Box
                                  ...
                                  sx={{
                                    ...
                                    '&:hover': {
                                      ...
                                      borderColor: `color-mix(in srgb, ${trigger.color} 50%, transparent)`,
                                      transform: 'translateY(-2px)',
                                      boxShadow: `0 4px 12px color-mix(in srgb, ${trigger.color} 20%, transparent)`,
                                    },
                                  }}
                                >
                                  <Box
                        ...
                        >
                          ...
                          {canEdit && (
                            <Button
                              ...
                              tone='secondary'
                              size='sm'
                              icon={<AddIcon sx={{ fontSize: 'var(--ds-text-title)' }} />}
                          ...
                          {canEdit && !isNewWorkflow && (
                            <Box sx={{ display: 'flex', alignItems: 'center' }} data-testid='run-split-button'>
                              <Button
                                ...
                                onClick={() => handleRunButtonClick('current')}
                              >
                          ...
                          {canEdit && isTestRunning && !isNewWorkflow && (
                            <Button
                          ...
                          {canEdit && (
                            <Button
                              ...
                              tone='secondary'
                              size='sm'
                              icon={<SafeIcon src={PlayIconBlue} alt='Dry Run' width={16} height={16} />}
                          ...
                          {nubiChatFeatureEnabled && (
                            <Button
                              ...
                              aria-label={`Ask ${assistantName} AI`}
                              icon={<SafeIcon src={getNubiIconUrl()} alt='AI Chat' width={22} height={22} />}
                              onClick={() => setShowNubiChat(!showNubiChat)}
                            />
                          )}
                          ...
                          <Button
                            ...
                            icon={<SafeIcon src={SettingOutlineIconGrey} alt='Settings' width={20} height={20} />}
                          ...
                          <Button
                            ...
                            icon={<AutoFixHighIcon sx={{ fontSize: 'var(--ds-text-heading)' }} />}
              ...
              {triggerConfigSidebarOpen && (
                <Suspense fallback={null}>
                  <TriggerConfigSidebar
                    ...
                    onRequestCloseWithUnsaved={(config) => {
                      const nodeId = selectedNode?.id;
                      if (nodeId) {
                        setPendingTriggerDiscard({ nodeId, pendingConfig: config });
              ...
              {/* Trigger sidebar close-confirmation (sibling, not nested) */}
              <Modal
                open={!!pendingTriggerDiscard && !triggerConfigSidebarOpen}
                handleClose={() => {
                  setTriggerConfigSidebarOpen(true);
                }}
                title='You have unsaved changes'
                ...
                actionButtons={
                  <Box sx={{ display: 'flex', gap: 2, justifyContent: 'flex-end', p: 2 }}>
                    ...
                    <Button
                      id='trigger-close-confirm-discard-btn'
                      tone='danger'
                      size='md'
                      onClick={() => {
                        setPendingTriggerDiscard(null);
                        closeTriggerConfigSidebar(setNodes);
                      }}
                    >
                      Discard changes
                    ...
                    <Button
                      id='trigger-close-confirm-save-btn'
                      tone='primary'
                      size='md'
                      onClick={() => {
                        if (pendingTriggerDiscard) {
                          updateTriggerConfig(pendingTriggerDiscard.nodeId, pendingTriggerDiscard.pendingConfig, setNodes);
                        }
                        setPendingTriggerDiscard(null);
                        closeTriggerConfigSidebar(setNodes);
                      }}
          ...
          <Drawer anchor='right' open={historyOpen} onClose={() => setHistoryOpen(false)}>
            <Box sx={{ width: 420, display: 'flex', flexDirection: 'column', height: '100%' }}>
              <Box sx={{ p: 2, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <Typography variant='h6' sx={{ fontWeight: 'var(--ds-font-weight-semibold)' }}>
                  Version History
                </Typography>
                <Button
                  composition='icon-only'
          ...
          <Modal
            open={!!confirmRestoreVersion}
            handleClose={() => (restoring ? undefined : setConfirmRestoreVersion(null))}
            width='sm'
            ...
            actionButtons={
              <Box sx={{ display: 'flex', justifyContent: 'flex-end', gap: 1, p: 2 }}>
                ...
                <Button
                  id='workflow-restore-confirm-btn'
                  tone='primary'
                  size='md'
                  ...
                  loading={restoring}
                >
                  {restoring ? 'Checking out…' : 'Checkout'}
                </Button>
              </Box>
            }
          >
          ...
          <Modal
            ...
            actionButtons={
              <Box sx={{ display: 'flex', justifyContent: 'flex-end', gap: 1, p: 2 }}>
                ...
                <Button
                  id='workflow-publish-confirm-btn'
                  tone='primary'
                  size='md'
          ...
          >
            <Box sx={{ p: 'var(--ds-space-4) 0' }}>
              ...
              <Box data-testid='workflow-publish-name-input' sx={{ mb: 1.5 }}>
                <Input
                  size='sm'
                  label='Name (optional)'
              ...
              <Box data-testid='workflow-publish-description-input'>
                <Input
                  size='sm'
          ...
          <Modal
            open={!!confirmLiveVersion}
            handleClose={() => (settingLive ? undefined : setConfirmLiveVersion(null))}
            width='sm'
            ...
            actionButtons={
              <Box sx={{ display: 'flex', justifyContent: 'flex-end', gap: 1, p: 2 }}>
                ...
                <Button
                  id='workflow-make-live-confirm-btn'
                  tone='primary'
                  size='md'
