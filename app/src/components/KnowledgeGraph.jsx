    ...
    return (
      <div className={`service-node-wrapper ${isZoomedOut ? 'lod-dot' : 'lod-full'}`}>
        ...
        <div className='service-node' style={{ borderColor }}>
          ...
          <Button
            className='focus-btn'
            icon={<FilterCenterFocusIcon style={{ fontSize: 14 }} />}
            tone='secondary'
            size='xs'
      ...
      >
        ...
        {fullPath.map((node, index) => (
          <Box key={`${node.value}-${index}`} sx={{ display: 'flex', alignItems: 'center', gap: 'var(--ds-space-1)' }}>
            {index > 0 && <ChevronRightIcon sx={{ fontSize: 16, color: 'var(--ds-gray-400)' }} />}
            <Tooltip
              ...
              >
                ...
                {node.isUserSelected && (
                  <CloseIcon
                    ...
                    sx={{ fontSize: 14, color: 'var(--ds-gray-600)', cursor: 'pointer', '&:hover': { color: 'var(--ds-gray-700)' } }}
                  />
                )}
              </Box>
            </Tooltip>
        ...
        {hasEdges && fullPath.length > 1 && (
          ...
          >
            <InfoOutlinedIcon sx={{ fontSize: 16, marginRight: 'var(--ds-space-1)' }} />
          ...
          <Box sx={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 'var(--ds-space-1)' }}>
            {fullPath.map((node, index) => (
              <Box key={`detail-${node.value}-${index}`} sx={{ display: 'flex', alignItems: 'center', gap: 'var(--ds-space-1)' }}>
                ...
                {index > 0 && !node.edgeFromPrev && <ChevronRightIcon sx={{ fontSize: 14, color: 'var(--ds-gray-400)' }} />}
            ...
            {focusedNodeId && (
              ...
              >
                ...
                <CloseIcon
                  ...
                  sx={{ fontSize: 14, cursor: 'pointer', ml: ds.space[0], opacity: 0.85, '&:hover': { opacity: 1 } }}
