...
export interface ChipProps {
  ...
  /** When true, truncates string children at `tooltipCharLimit` chars and shows full text in a tooltip on hover. */
  displayTooltip?: boolean;
  /** Character limit for truncation when `displayTooltip` is true. Defaults to 30. */
...
//
// Categorical hues map to the 8 DS color families (gray/green/amber/red/blue/
// purple/pink/teal), using the same bg=100 / fg=700 / border=200 ramp as the
// semantic tones. They stay a distinct *categorical* system — a "category"
// must never read as a "state" — but source from --ds-* tokens, not raw hex,
// per the DS tokens-only rule.
...
const HUES: Record<ChipHue, HueSet> = {
  slate: { bg: 'var(--ds-gray-100)', fg: 'var(--ds-gray-700)', border: 'var(--ds-gray-200)' },
  green: { bg: 'var(--ds-green-100)', fg: 'var(--ds-green-700)', border: 'var(--ds-green-200)' },
  amber: { bg: 'var(--ds-amber-100)', fg: 'var(--ds-amber-700)', border: 'var(--ds-amber-200)' },
  red: { bg: 'var(--ds-red-100)', fg: 'var(--ds-red-700)', border: 'var(--ds-red-200)' },
  blue: { bg: 'var(--ds-blue-100)', fg: 'var(--ds-blue-700)', border: 'var(--ds-blue-200)' },
  violet: { bg: 'var(--ds-purple-100)', fg: 'var(--ds-purple-700)', border: 'var(--ds-purple-200)' },
  pink: { bg: 'var(--ds-pink-100)', fg: 'var(--ds-pink-700)', border: 'var(--ds-pink-200)' },
  teal: { bg: 'var(--ds-teal-100)', fg: 'var(--ds-teal-700)', border: 'var(--ds-teal-200)' },
};
...
export function Chip(props: ChipProps) {
  ...
  const shouldShowTooltip = !!(displayTooltip && childrenText && childrenText.length > tooltipCharLimit);
  const displayChildren = shouldShowTooltip ? `${childrenText.substring(0, tooltipCharLimit)}...` : children;
  ...
  const content = (
    <>
      ...
      {hasChildren && <Box component='span'>{displayChildren}</Box>}
      {!isCountFirst && countNode}
      {trailingNode}
    </>
  );
  ...
  const chipElement = isInteractive ? (
    <ButtonBase
      id={id}
      className={className}
      onClick={onClick}
  ...
  if (shouldShowTooltip && childrenText) {
    return (
      <Tooltip title={childrenText} placement='top'>
        {chipElement}
      </Tooltip>
    );
  }
