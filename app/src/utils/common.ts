...
export function safeJSONParse(data: string) {
  ...
  // LogQL/PromQL selector heuristic: `{` immediately followed by an
  // unquoted identifier and =, !=, =~, or !~. JSON keys must be quoted.
  ...
  try {
    ...
    // safeJSONParse contract: return null on failure, don't log.
    // (Next.js dev overlay elevates console.error with an Error object
    // to a full-screen popup, which is hostile for an opportunistic parser.)
    return null;
  }
}
