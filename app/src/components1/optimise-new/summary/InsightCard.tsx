...
const TruncatedTooltip = ({
  ...
  return (
    <Tooltip title={title} placement={placement} open={open} onClose={handleLeave}>
      {child}
    </Tooltip>
  );
};
    ...
    >
      ...
      <Box sx={{ flex: 1, minWidth: 0 }}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: ds.space[1], mt: 'var(--ds-space-1)', flexWrap: 'wrap' }}>
          ...
          {item.resourceId && (
            <Box sx={{ ...metaPillSx, maxWidth: ds.space.mul(0, 110) }}>
          ...
          {item.accountName && (
            <>
              <Dot />
              <Box sx={{ display: 'flex', alignItems: 'center', gap: ds.space[1], flexShrink: 0, maxWidth: ds.space.mul(0, 110) }}>
      ...
      <Box sx={{ textAlign: 'right', minWidth: ds.space.mul(0, 45), flexShrink: 0 }}>
        {item.impactValue && (
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: ds.space[1] }}>
          {onAskNubi && (
            <Tooltip title='Ask Nubi' placement='top'>
              <Button
                tone='ghost'
                size='xs'
