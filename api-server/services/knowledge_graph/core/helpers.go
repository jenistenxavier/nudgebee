...
func DeduplicateEdgesWithPriority(edges []*DbEdge) []*DbEdge {
	...
	for _, edge := range edges {
		...
		if existing, exists := edgeMap[compositeKey]; exists {
			...
			// Seed the first-class field for new edges; the legacy
			// properties["contributing_sources"] slot is derived from this
			// field at the end so the two views can never drift.
			if edge.Properties == nil {
				edge.Properties = make(map[string]interface{})
			}
			if len(edge.ContributingSources) == 0 && edge.Source != "" {
				edge.ContributingSources = []EdgeContributingSource{
					{Source: edge.Source, LastSeenAt: time.Now()},
				}
			}
			syncContributingSourcesProperty(edge)
			edgeMap[compositeKey] = edge
		}
	}
...
func mergeEdgePropertiesWithSourcePrefix(primary, secondary *DbEdge) {
	...
	// Track contributing sources on the first-class field, persisted to the
	// contributing_sources jsonb column. One entry per source; LastSeenAt is
	// refreshed every time that source re-asserts the edge. Seed primary's
	// own source first so the swap path (where a higher-priority edge
	// displaces a lower-priority existing one) doesn't lose the new winner's
	// identity in the contributors list. Then pull through any contributors
	// the secondary already accumulated (e.g. an edge that dedup-merged in a
	// previous build cycle and was re-read from DB before this pass). The
	// legacy properties["contributing_sources"] slot is then derived from the
	// field so the two views cannot drift.
	now := time.Now()
	mergeContributingSourceField(primary, primary.Source, now)
	mergeContributingSourceField(primary, secondary.Source, now)
	for _, cs := range secondary.ContributingSources {
		mergeContributingSourceField(primary, cs.Source, cs.LastSeenAt)
	}
	syncContributingSourcesProperty(primary)

	primary.UpdatedAt = time.Now()
}

// syncContributingSourcesProperty projects the first-class
// ContributingSources field down to the legacy properties slot so any
// downstream consumer reading from edge.Properties["contributing_sources"]
// sees the same list as the JSONB column. Field is the source of truth.
func syncContributingSourcesProperty(edge *DbEdge) {
	if edge == nil {
		return
	}
	if edge.Properties == nil {
	...
	sources := make([]string, 0, len(edge.ContributingSources))
	for _, cs := range edge.ContributingSources {
		if cs.Source == "" {
			continue
		}
	...
	edge.Properties["contributing_sources"] = sources
}

// mergeContributingSourceField inserts or refreshes one contributor entry on
// the edge's first-class ContributingSources field. Empty source names are
// ignored. When the source already exists in the list, its LastSeenAt is
// updated to the later of the two timestamps so cycle-to-cycle re-stamps move
// the timestamp forward without ever moving it backward.
func mergeContributingSourceField(edge *DbEdge, source string, seenAt time.Time) {
	if source == "" {
		return
	}
	for i := range edge.ContributingSources {
		if edge.ContributingSources[i].Source == source {
			if seenAt.After(edge.ContributingSources[i].LastSeenAt) {
				edge.ContributingSources[i].LastSeenAt = seenAt
			}
			return
		}
	}
	edge.ContributingSources = append(edge.ContributingSources, EdgeContributingSource{
		Source:     source,
		LastSeenAt: seenAt,
	})
}
...
var nodeTypeLogoMap = map[NodeType]string{
	...
	NodeTypeNode:               "node",
	NodeTypePod:                "pod",
	NodeTypeK8sServiceAccount:  "k8sserviceaccount",
