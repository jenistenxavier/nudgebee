...
func TestApplyLabelMapping(t *testing.T) {
	...
	newEvent := func(subject, namespace string, sev eventtypes.EventPriortiy, labels map[string]string) EventIncomingWebhook {
	...
	t.Run("namespace and severity override + normalize", func(t *testing.T) {
		...
		event := newEvent("svc", "old-ns", eventtypes.EventPriortiyLow, map[string]string{"k8s_ns": "team-ns", "prio": "critical"})
		applyLabelMapping(&event, mapping, newLabelExtractorCache(logger, mapping), true)
		assert.Equal(t, "team-ns", event.EventSubjectNamespace)
		assert.Equal(t, eventtypes.EventPriortiyHigh, event.Investigation.Severity)
	})
