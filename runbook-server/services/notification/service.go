...
//
// Recipients acts as the To addresses. CC and BCC are optional. When CC or
// BCC is non-empty, the notifications-server sends a single combined message
// with To/Cc headers; BCC addresses receive the message but never appear in
// the sent message's headers.
...
func SendEmail(ctx *security.RequestContext, request SendEmailRequest) (SendEmailResponse, error) {
	...
	if len(request.CC) > 0 {
		requestBody["cc"] = request.CC
	}

	if len(request.BCC) > 0 {
		requestBody["bcc"] = request.BCC
	}
