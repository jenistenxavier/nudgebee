...
func AddTicketConfiguration(ctx *gin.Context) {
	...
	// Fail closed on a missing tenant before any DB lookup or persist, so we can't
	// read/write cross-tenant or insert an orphaned integration. Mirrors the guard
	// in TestConnectionByConfig.
	if configuration.Tenant == "" {
		ctx.JSON(http.StatusBadRequest, gin.H{"message": "tenant_id is required"})
		return
	}
	...
	if configuration.Password == "" || configuration.AuthType == "" {
		...
		if existing.Found {
			...
			if configuration.AuthType == "" {
				configuration.AuthType = existing.AuthType
			}
		}
	}
...
func CreateTicketAction(c *gin.Context) {
...
func CreateTicket(c *gin.Context) {
	...
	ticket.CreatedBy = headerUserID(c)
	ticket.Tenant = headerTenantID(c)
...
// TestConnectionByConfig validates ticketing credentials before they are persisted.
// The request body mirrors AddTicketConfiguration. On edit (configuration.ID set),
// an empty password is rehydrated from the stored encrypted value so users can
// re-test without re-entering their token — same convention as the save path.
func TestConnectionByConfig(ctx *gin.Context) {
	var req models.TestConnectionByConfigRequest
	if err := ctx.ShouldBindJSON(&req); err != nil {
		ctx.JSON(http.StatusOK, models.TestConnectionResponse{
			Success: false,
	...
	configuration := req.Input.Object
	configuration.Tenant = req.SessionVariables.UserTenantID
	configuration.Name = strings.TrimSpace(configuration.Name)
	...
	// The frontend omits the password on re-test (keep stored secret) and never
	...
	if configuration.Password == "" || configuration.AuthType == "" {
		...
		if err != nil {
			...
			ctx.JSON(http.StatusOK, models.TestConnectionResponse{
				Success: false,
				Tool:    configuration.Tool,
				Error:   fmt.Sprintf("failed to load existing integration: %v", err),
			})
			return
		}
		if existing.Found {
	...
	ctx.JSON(http.StatusOK, models.TestConnectionResponse{
		Success: true,
		Tool:    configuration.Tool,
		Message: fmt.Sprintf("Successfully connected to %s", configuration.Tool),
	})
}
...
func ListTickets(c *gin.Context) {
	...
	userID := headerUserID(c)
	tenantID := headerTenantID(c)
...
func bindAndAuthoriseTicketRequest(ctx *gin.Context, permission string) (models.Ticket, bool) {
	...
	if req.SessionVariables.Role != "admin" {
		auth := &common.Authorization{
			UserID:     ticket.CreatedBy,
			TenantID:   ticket.Tenant,
			AccountID:  ticket.AccountID,
